...
var TimeGridEventRenderer = /** @class */ (function (_super) {
    ...
    async function getValor(id){
        ...
        var sql = "SELECT valor FROM events WHERE `id`=? ";
        var params = [];
        params.push(id);
        sql = mysql.format(sql, params);
         ...
         await pool.getConnection(async function(err, connection) {
          ...
          await  connection.query(sql, function(err, rows) {
              if (err) return next(err);
              else {myValor=rows[0].valor;  console.log(id+"WE: "+rows[0].valor); document.getElementById('valor'+id+'').innerHTML += rows[0].valor; return(myValor);}
              connection.release();
            });
          });
            ...
            (eventDef.title ?
                '<div class="fc-title">' +
                    util_1.htmlEscape(eventDef.title) +
                     '<div class="fc-valor" id="valor'+eventDef.id+'">R$ '+
                     '</div>'  +'</div>':
                '') +
            '</div>' +
            '<div class="fc-bg"/>' +
